...
Graph createAGraph(int v, int e) {
    ...
    MALLOC(graph->visited, e * sizeof(int));
    for(int i = 0; i < e; i++)
...
void DAG(Graph graph, int v){
    ...
    while(front != rear){
        int temp = deleteq();
        if(graph->edge[temp].weight + subset[graph->edge[temp].src].rank < subset[graph->edge[temp].dest].rank){
            subset[graph->edge[temp].dest].parent = graph->edge[temp].src;
            subset[graph->edge[temp].dest].rank = graph->edge[temp].weight + subset[graph->edge[temp].src].rank;
        }
        for(int i = 0; i < E; i++)
            if(graph->edge[i].src == graph->edge[temp].dest && graph->visited[i] == FALSE){
                graph->visited[i] = TRUE;
                addq(i);
            }
        // printQueue();
